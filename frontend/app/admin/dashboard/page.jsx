...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      {/* Page Header */}
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 sm:from-white sm:to-white rounded-2xl px-5 py-6 sm:py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5 sm:gap-4 shadow-sm sm:border sm:border-slate-200">
        <div>
          <h1 className="text-2xl sm:text-xl font-bold text-white sm:text-slate-900 tracking-tight">Dashboard</h1>
          <p className="text-sm text-blue-100 sm:text-slate-500 mt-1">
        ...
        <div className="flex w-full sm:w-auto gap-3">
          <Button
            ...
            variant="outline"
            className="flex-1 sm:flex-none gap-2 border-white/30 sm:border-slate-200 text-white sm:text-slate-600 hover:bg-white/10 sm:hover:bg-slate-50 bg-white/10 sm:bg-transparent"
          >
          ...
          <Button
            ...
            className="flex-1 sm:flex-none gap-2 bg-white sm:bg-blue-600 text-blue-600 sm:text-white hover:bg-blue-50 sm:hover:bg-blue-700 shadow-none"
          >
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        {statsData.map(({ title, value, subtitle, icon: Icon, accent, iconBg }) => (
          <div key={title} className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-200 border-l-4 border-l-blue-500 flex items-center justify-between sm:block">
            <div className="sm:mb-4">
              <span className="text-[11px] sm:text-xs font-semibold text-slate-500 uppercase tracking-wide leading-none">{title}</span>
              <p className={`text-2xl sm:text-3xl font-bold mt-1.5 sm:mt-0 ${accent}`}>{value}</p>
              <p className="text-[11px] sm:text-xs text-slate-400 mt-1">{subtitle}</p>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-4">
            {quickActions.map(({ label, icon: Icon, path }) => (
              <button
                ...
                className="bg-white border border-slate-200 rounded-xl p-3 sm:p-5 flex flex-col items-center justify-center gap-2 sm:gap-3 hover:border-blue-300 hover:bg-blue-50/40 transition-all duration-150 group shadow-sm hover:shadow-md"
              >
                <div className="p-2.5 sm:p-3 rounded-xl bg-slate-100 group-hover:bg-blue-100 border border-slate-200 group-hover:border-blue-200 transition-colors">
                  <Icon className="h-4 w-4 sm:h-5 sm:w-5 text-slate-500 group-hover:text-blue-600 transition-colors" />
                </div>
                <span className="text-[10px] sm:text-sm font-medium text-slate-700 group-hover:text-blue-700 transition-colors text-center leading-tight">{label}</span>
              </button>
            ))}
          </div>
        </div>
