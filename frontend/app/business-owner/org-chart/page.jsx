...
"relative w-[260px] bg-card border rounded-xl p-4 shadow-sm transition-all duration-200 cursor-pointer hover:shadow-md hover:border-blue-300",
 ...
 {/* Department Icon / Manager Badge */}
 {data.isManager && <div className="absolute top-2 right-8 p-1"><Shield className="h-4 w-4 text-purple-500" /></div>}
...
const OrgChartFlow = ({ treeData, expandedNodes, toggleNode, searchTerm, onNodeClick, selectedNodeId }) => {
 ...
 bgColor: node.type === 'org' ? 'bg-blue-100 dark:bg-blue-900/40' : node.type === 'dept' ? 'bg-slate-100 dark:bg-slate-800' : node.isManager ? 'bg-purple-100 dark:bg-purple-900/40' : 'bg-secondary',
 textColor: node.type === 'org' ? 'text-blue-700 dark:text-blue-300' : node.type === 'dept' ? 'text-slate-700 dark:text-slate-300' : node.isManager ? 'text-purple-700 dark:text-purple-300' : 'text-foreground',
 isExpanded,
 hasChildren,
 isMatch,
 ...
 <span className="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full mt-1 border border-purple-200">Manager</span>
...
export default function BusinessOwnerOrgChartPage() {
 ...
 if (item.manager) {
 managerNode = {
 id: `user-${item.manager.id}`,
 type: 'user',
 name: item.manager.name,
 subtitle: item.manager.position || 'Manager',
 isManager: true,
 ...item.manager,
 children: []
 }
 ...
 // Attach employees to the Manager if they exist, otherwise directly to Department
 if (managerNode) {
 managerNode.children = [...employeesList]
 } else {
