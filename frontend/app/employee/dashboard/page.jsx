...
function ProgressRing({ radius, stroke, progress, total, colorClass, label, sublabel }) {
 ...
 <circle stroke="currentColor" fill="transparent" strokeWidth={stroke} r={nr} cx={radius} cy={radius} className="text-slate-100 dark:text-foreground" />
 ...
 <span className="text-xl font-bold text-foreground tracking-tight leading-none">{label}</span>
 {sublabel && <span className="text-[9px] text-muted-foreground font-bold uppercase tracking-widest mt-1">{sublabel}</span>}
 </div>
 </div>
 )
}
...
export default function EmployeeDashboardPage() {
 ...
 <div className="text-center"><RefreshCw className="h-10 w-10 animate-spin mx-auto mb-4 text-blue-500" /><p className="text-muted-foreground font-medium">Loading workspace...</p></div>
 </div>
 )
 ...
 <div className="absolute -top-20 -right-20 w-80 h-80 bg-card/10 rounded-full blur-3xl" />
 ...
 <div className="inline-flex items-center gap-2.5 bg-card/10 backdrop-blur-md px-4 py-2 rounded-full border border-white/20">
 ...
 {todayRecord?.checkIn && (
 <div className="inline-flex items-center gap-2 bg-card/10 px-3 py-1.5 rounded-full border border-white/20 text-blue-100 text-xs font-semibold">
 ...
 <AvatarFallback className="bg-card/20 text-white text-lg font-bold">
 ...
 {profile?.department && (
 <div className="flex items-center gap-1 text-xs text-blue-100 bg-card/10 px-2 py-0.5 rounded-full">
 ...
 {todayRecord?.checkIn && (
 <p className="text-xs text-muted-foreground font-medium">In: <span className="text-foreground font-bold">{todayRecord.checkIn}</span>{todayRecord?.checkOut ? <> &middot; Out: <span className="text-foreground font-bold">{todayRecord.checkOut}</span></> : ''}</p>
 )}
 {todayRecord?.breakDuration ? (
 <p className="text-xs text-muted-foreground font-medium">Break: {Math.floor(todayRecord.breakDuration / 60)}h {todayRecord.breakDuration % 60}m</p>
 ) : null}
 </div>
 ...
 <span className="text-[9px] font-bold text-muted-foreground group-hover:text-slate-600 transition-colors uppercase mt-1.5 tracking-widest">{type}</span>
 </div>
 </TooltipTrigger>
 ...
 <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-2">Upcoming Leave</p>
 ...
 <div className="hidden sm:flex items-center gap-2.5 text-[9px] font-bold text-muted-foreground uppercase tracking-wide">
 <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm bg-secondary border border-border inline-block" /> Absent</span>
 ...
 <Badge variant="secondary" className="bg-secondary text-muted-foreground hover:bg-secondary font-bold text-[10px] tracking-wider">30 DAYS</Badge>
 </CardHeader>
 ...
 <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">{dayName}</span>
 </div>
 </td>
 ...
 {ei !== record.events.length - 1 && <div className="w-px flex-1 bg-secondary my-1" />}
 </div>
 <div>
 <p className="font-bold text-foreground capitalize">{evt.type.replace(/([A-Z])/g, ' $1').trim()}</p>
 <div className="flex items-center gap-1 mt-0.5 text-muted-foreground font-medium">
 <Clock className="h-3 w-3" />
 {format(parseISO(evt.time), 'hh:mm a')}
 </div>
 {evt.location && (
 <p className="text-muted-foreground mt-0.5 flex items-start gap-1 leading-tight">
 ...
 )) || <p className="text-muted-foreground text-xs font-medium">No events logged.</p>}
 </div>
 </TooltipContent>
 </Tooltip>
 </TooltipProvider>
